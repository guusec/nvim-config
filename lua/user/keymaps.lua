...
vim.keymap.set('n', '<leader>cc', '<cmd>CodeCompanionChat toggle<cr>', {desc = 'Code Companion prompt'})
...
--[[vim.keymap.set('n', '<leader><CR>', function()
  ...
  if term_winnr and vim.api.nvim_win_is_valid(term_winnr) then
    -- Close the terminal
    vim.api.nvim_win_close(term_winnr, false)
    term_winnr = nil
  else
    -- Open terminal in vertical split
    vim.cmd('botright vsplit')
    term_winnr = vim.api.nvim_get_current_win()
    
    if term_bufnr and vim.api.nvim_buf_is_valid(term_bufnr) then
      -- Use existing terminal buffer
      vim.api.nvim_win_set_buf(term_winnr, term_bufnr)
    else
      -- Create new terminal and run claude
      vim.cmd('terminal /home/giuseppe/.claude/local/claude')
...
-- Also allow toggling from within the terminal
vim.keymap.set('t', '<leader><CR>', function()
  if term_winnr and vim.api.nvim_win_is_valid(term_winnr) then
    vim.api.nvim_win_close(term_winnr, false)
    term_winnr = nil
  end
end, { desc = 'Close terminal from within', silent = true })

-- Reduce timeout specifically for terminal mode to make spacebar more responsive
vim.api.nvim_create_autocmd('TermOpen', {
  pattern = '*',
  callback = function()
    vim.opt_local.timeoutlen = 100  -- Much faster in terminal
  end
}) ]]--
